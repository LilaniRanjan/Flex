...
try {
    ...
    echo "Error in PortionEdit File Db Connection: " . $exc->getMessage();
}
?>
            ...
            <main class="main-container">
                ...
                try {
                    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit"])) {
                        if (isset($_POST['portion_size_Id'], $_POST['portion_size_name'])) {
                            $portion_size_Id = $_POST['portion_size_Id'];
                            $portion_size_name = $_POST['portion_size_name'];

                            try {
                                $portion_detail = new \classes\PortionSize($portion_size_name);
                                $portion = $portion_detail->updatePortionDetails($con, $portion_size_Id);
                                if ($portion) {
                        ...
                        if (isset($_GET['id'])) {
                            ...
                            $portion_detail = $portion_obj->getPortionDetailById($con, $portion_size_Id);

                            if ($portion_detail) {
                                ?>
                                <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" enctype="multipart/form-data">
                                    <div id="cardStyle" class="card mx-auto shadow p-3 mb-5 rounded" style="width: 50%;">
                                        <div class="card-header text-center"><h4 style="color: #E88F2A;">EDIT SPICY LEVEL DETAILS</h4></div>
                                        <hr>
                                        <div class="card-body">
                                            <div class="card-body">
                                                <div class="form-group my-2">
                                                    <input name="portion_size_Id" type="hidden" id="portion_size_Id" class="form-control" value="<?php echo $portion_size_Id; ?>" required/>  
                                                </div>
                                                <div class="form-group my-2">
                                                    <label for="portion_size_name">Spicy Level Name :</label>
                                                    <input style="background-color: black; color: white;" name="portion_size_name" type="text" id="portion_size_name" class="form-control" value="<?php echo $portion_detail->getPortion_size_name(); ?>" required/>  
                                                </div>
                                                <br>
                                                <br>
                                                <button type="submit" name="submit" class="mt-3 btn btn-lg btn-block form-control" style="background-color: #333; color: white; border: 1px solid #E88F2A;">UPDATE PORTION SIZE</button>
                                            </div>
                                        </div>
                                    </div>
                                </form>
                    ...
                    echo "Error in Update Spicy Level details: " . $exc->getMessage();
                }
                ?>
