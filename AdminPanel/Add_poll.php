        ...
        <div class="grid-container">
            ...
            <main class="main-container">
                ...
                if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit"])) {
                    if (isset($_POST['date'], $_POST['food_name'])) {
                        $poll_created_date = $_POST['date'];
                        $food_name = $_POST['food_name'];

                        try {
                            $poll_detail = new \classes\Poll($poll_created_date, $food_name);
                            $poll = $poll_detail->getAddPoll($dbcon);
                           if ($poll) {
                           ...
                           echo "An error occurred: " . $exc->getMessage(); 
...
?>
                ...
                <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" enctype="multipart/form-data">
                    <div id="cardStyle" class="card mx-auto shadow p-3 mb-5 rounded" style="width: 50%;">
                        <div class="card-header text-center"><h4 style="color: #E88F2A;">ADD FOOD FOR POLL</h4></div>
                        <hr>
                        <div class="card-body">
                            <div class="card-body">
                                <div class="form-group my-2">
                                    <label for="date">Poll Created Date :</label>
                                    <input style="background-color: black; color: white;" name="date" type="date"  class="form-control" required/>  
                                </div>
                                <br>
                                <div class="form-group my-2">
                                    <label for="food_name">Food Name :</label>
                                    <input style="background-color: black; color: white;" name="food_name" type="text"  class="form-control" required/>
                                </div>
                                <br>
                                <br>
                                <button type="submit" name="submit" class="mt-3 btn btn-lg btn-block form-control" style="background-color: #333; color: white; border: 1px solid #E88F2A;">ADD FOOD</button>
                            </div>
                        </div>
                    </div>
                </form>
