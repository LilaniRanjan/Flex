...
try {
    ...
    echo "Error in SpicyEdit File Db Connection: " . $exc->getMessage();
}
?>
            ...
            <main class="main-container">
                ...
                try {
                    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit"])) {
                        if (isset($_POST['spice_level_Id'], $_POST['spice_level'])) {
                            $spice_level_Id = $_POST['spice_level_Id'];
                            $spice_level = $_POST['spice_level'];

                            try {
                                $spicy_detail = new classes\SpiceLevel($spice_level);
                                $spicy = $spicy_detail->updateSpicyDetails($con, $spice_level_Id);
                                if ($spicy) {
                        ...
                        if (isset($_GET['id'])) {
                            $spice_level_Id = $_GET["id"];
                            $spicy_obj = new classes\SpiceLevel(null);
                            $spicy_detail = $spicy_obj->getSpicyDetailById($con, $spice_level_Id);
                                        ...
                                        <div class="card-body">
                                            <div class="card-body">
                                                <div class="form-group my-2">
                                                    <input name="spice_level_Id" type="hidden" id="spice_level_Id" class="form-control" value="<?php echo $spice_level_Id; ?>" required/>  
                                                </div>
                                                <div class="form-group my-2">
                                                    <label for="spice_level">Spicy Level Name :</label>
                                                    <input style="background-color: black; color: white;" name="spice_level" type="text" id="spice_level" class="form-control" value="<?php echo $spicy_detail->getSpice_level(); ?>" required/>  
                                                </div>
                                                <br>
                                                <br>
                                                <button type="submit" name="submit" class="mt-3 btn btn-lg btn-block form-control" style="background-color: #333; color: white; border: 1px solid #E88F2A;">UPDATE SPICY LEVEL</button>
                                            </div>
                                        </div>
                                    </div>
                                </form>
