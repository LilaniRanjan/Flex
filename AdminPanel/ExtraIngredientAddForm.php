        ...
        <div class="grid-container">
            ...
            <main class="main-container">
                ...
                if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit"])) {
                    if (isset($_POST['extra_ingredients_name'], $_POST['extra_ingredients_price'])) {
                        $extra_ingredients_name = $_POST['extra_ingredients_name'];
                        $extra_ingredients_price = $_POST['extra_ingredients_price'];

                        try {
                            $extra_ingredients_detail = new \classes\ExtraIngredients($extra_ingredients_name, $extra_ingredients_price);
                            $extra_ingredient = $extra_ingredients_detail->addExtraIngredientDetails($con);
                            if ($extra_ingredient) {
                ...
                <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" enctype="multipart/form-data">
                    <div id="cardStyle" class="card mx-auto shadow p-3 mb-5 rounded" style="width: 50%;">
                        <div class="card-header text-center"><h4 style="color: #E88F2A;">ADD NEW CURRY DETAILS</h4></div>
                        <hr>
                        <div class="card-body">
                            <div class="card-body">
                                <div class="form-group my-2">
                                    <label for="extra_ingredients_name">Extra Ingredient Name :</label>
                                    <input style="background-color: black; color: white;" name="extra_ingredients_name" type="text" id="extra_ingredients_name" class="form-control" required/>  
                                </div>
                                <br>
                                <div class="form-group my-2">
                                    <label for="extra_ingredients_price">Extra Ingredient Price :</label>
                                    <input style="background-color: black; color: white;" name="extra_ingredients_price" type="number" id="extra_ingredients_price" class="form-control" required/>
                                </div>
                                <br>
                                <br>
                                <button type="submit" name="submit" class="mt-3 btn btn-lg btn-block form-control" style="background-color: #333; color: white; border: 1px solid #E88F2A;">ADD EXTRA INGREDIENT</button>
                            </div>
                        </div>
                    </div>
                </form>
