...
try {
    ...
    echo "Error in CurryEdit File Db Connection: " . $exc->getMessage();
}
?>
            ...
            <main class="main-container">
                ...
                try {
                    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit"])) {
                        if (isset($_POST['curry_name'], $_POST['curry_price'], $_POST['curry_id'])) {
                            $curry_id = $_POST['curry_id'];
                            $curry_name = $_POST['curry_name'];
                            $curry_price = $_POST['curry_price'];

                            try {
                                $curry_detail = new \classes\CurryDetails($curry_name, $curry_price);
                                $curry = $curry_detail->updateCurryDetails($con, $curry_id);
                                if ($curry) {
                        ...
                        if (isset($_GET['id'])) {
                            ...
                            $curry_detail = $curry_obj->getCurryDetailById($con, $curry_id);
                                        ...
                                        <div class="card-body">
                                            <div class="card-body">
                                                <div class="form-group my-2">
                                                    <input name="curry_id" type="hidden" id="curry_id" class="form-control" value="<?php echo $curry_id; ?>" required/>  
                                                </div>
                                                <div class="form-group my-2">
                                                    <label for="curry_name">Curry Name :</label>
                                                    <input style="background-color: black; color: white;" name="curry_name" type="text" id="curry_name" class="form-control" value="<?php echo $curry_detail->getCurry_name();?>" required/>  
                                                </div>
                                                <br>
                                                <div class="form-group my-2">
                                                    <label for="curry_price">Curry Price :</label>
                                                    <input style="background-color: black; color: white;" name="curry_price" type="number" id="curry_price" class="form-control" value="<?php echo $curry_detail->getCurry_price(); ?>" required/>
                                                </div>
                                                <br>
                                                <br>
